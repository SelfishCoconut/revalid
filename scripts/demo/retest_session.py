...
def _run_scripted_session(
    ...
    # 1. The operator runs a manual command (ungated) while the agent's proposal awaits.
    submit_human_command(session, registry, record.id, "whoami")
    # 2. Approve the agent's proposed command — it runs, the agent observes the
    #    operator's `!whoami`, and concludes with a verdict.
    apply_decision(
...
def _pending_cid(session: Session, session_id: int) -> str:
    ...
    proposal = next(event for event in reversed(events) if event["kind"] == "command_proposed")
