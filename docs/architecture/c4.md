...
C4Container
    ...
    System_Boundary(revalid, "revalid (localhost only — NFR-03)") {
        Container(spa, "React SPA", "Vite/React/TS/Tailwind (FR-11)", "Report overview; a finding stage wizard (extract | plan | approve | retest | verdict) with versioned finding edits + stage-tagged notes (FR-16); evidence drill-down")
        Container(api, "FastAPI app", "Python, /api", "Ingest, extraction, versioned finding revision + notes (FR-16), plan/approve gate, sanity-guarded retest executor (FR-08), audit re-derivation (FR-10), versioned export (FR-12); serves the SPA at /")
        ...
        ContainerDb(db, "SQLite", "SQLAlchemy", "Reports; findings (identity + append-only version rows) + notes; plan versions; verdicts + evidence + audit stamps (created_at / actor)")
    }
...
Since M5 the finding detail is a **stage wizard** (ADR-0024): the same
extract → plan → approve → retest → verdict operations shown below are reached as
five sub-routes (`/findings/{id}/{stage}`) the operator walks by clicking the
pipeline stepper — navigation only, never a mutation. FR-16 adds two sibling
`/api` actions on the finding itself: `POST /findings/{id}` records a versioned
finding edit (append-only, extraction = v1) and `POST /findings/{id}/notes`
appends a stage-tagged note; both are read back by the wizard.
