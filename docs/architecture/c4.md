...
C4Container
    ...
    System_Boundary(revalid, "revalid (localhost only — NFR-03)") {
        Container(spa, "React SPA", "Vite/React/TS/Tailwind (FR-11)", "Report overview (with a findings risk profile); a finding stage wizard (extract | goal | retest | verdict) with versioned finding edits + stage-tagged notes (FR-16); a live agentic retest-session console (chat + docked terminal, FR-17); a read-only reports chat over the corpus with streamed replies (FR-18)")
        Container(api, "FastAPI app", "Python, /api", "Ingest + extraction with CVSS/MITRE ATT&CK enrichment (FR-19), versioned finding revision + notes (FR-16), goal drafting, gated agentic retest sessions run inside an egress-locked sandbox (FR-17, ADR-0025), reports-chat corpus Q&A streamed over SSE (FR-18), audit re-derivation (FR-10), versioned export (FR-12); serves the SPA at /")
        ...
        ContainerDb(db, "SQLite", "SQLAlchemy", "Reports; findings (identity + append-only version rows, each carrying CVSS + MITRE ATT&CK) + notes; retest sessions + their append-only transcript (session_events); verdicts + evidence + audit stamps (created_at / actor); reports-chat threads (chat_sessions / chat_messages)")
    }
    ...
    Rel(auditor, spa, "Uploads report, sets the goal, approves each gated command, reads verdicts, asks the corpus chat", "localhost")
    Rel(spa, api, "JSON + WebSocket (retest console) + SSE (chat) over /api", "fetch / WS")
