...
- **FastAPI app** — ingest and extraction with CVSS/MITRE enrichment (FR-19);
...
flowchart TB
    ...
    subgraph UND["report understanding"]
        direction TB
        PDF["pdf.py<br/>FR-01"]
        EXT["extract.py<br/>FR-03"]
        IGS["ingest.py<br/>FR-02"]
        FND["findings.py<br/>FR-16, FR-19"]
    end
...
candidates into schema-validated findings; `ingest.py` covers the two LLM-free
doors (DefectDojo JSON and manual entry); `findings.py` owns versioned findings,
notes and the CVSS/MITRE enrichment. On the retest side, `plan.py` generates the
...
to the host the sandbox is provisioned against (FR-06, ADR-0041), and `deltas.py`
...
gets **proxy mode**: the session network stays `internal=True`, and its only
route out is a per-session Squid container configured deny-all-by-default with
the scoped host(s) as the sole `dstdomain` allowlist. Provisioning fails closed —
any error tears the session down rather than opening egress — and non-HTTP
traffic has no path out at all.
...
flowchart TB
    subgraph host["operator's machine"]
        ...
        subgraph proc["single uvicorn process — NFR-03<br/>on the host, or in the revalid-app container (ADR-0044)"]
            API["FastAPI /api"]
    ...
    subgraph net["per-session Docker network — revalid-retest-{id}, internal=true"]
        ...
        LAB["authorised lab container<br/>revalid-juice-shop"]
    end
...
> cannot block teardown. In proxy mode (an online scope host, ADR-0041) the only
> change to this picture is that the allowlisting Squid container joins the
> session network beside the sandbox and holds the sole route out; every dashed
> line stays dashed for anything it does not allowlist.
...
the same requirement is met by the allowlisting egress proxy of ADR-0041; the
network stays `internal` and the proxy is the only way out.) Three limits of that
...
sequenceDiagram
    ...
    Note over API,Agent: loop until a determination — a hand-back pauses for guidance (ADR-0034)
    Agent-->>API: ConcludeOutput(fixed | still_open)
