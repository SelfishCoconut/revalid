...
sequenceDiagram
    ...
    loop gated command loop (detail below) until verdict / given-up
...
sequenceDiagram
    ...
    Note over API,Agent: loop until ConcludeOutput or the step/wall-clock budget is exhausted
    Agent-->>API: ConcludeOutput(status, rationale) [or forced "budget exhausted"]
