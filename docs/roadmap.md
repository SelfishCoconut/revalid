...
- [x] PDF ingestion pipeline (FR-01) — `src/revalid/pdf.py` (pdfplumber, ADR-0007): text + best-effort finding candidates, fail-closed on non-PDF/corrupt/no-text. Validated against a synthetic fixture; **swapping in Álvaro's scrubbed real report is the remaining step to tick FR-01's acceptance box.**
