...
extra (`uv sync --extra sandbox`). Skips gracefully when Docker is missing —
mirroring the ``test_browser_xss_system.py`` precedent — so a developer
without a daemon still gets a green suite; CI (system-tests.yml) provisions
both. The acceptance: a real ``DockerSandbox`` can reach the allowlisted lab
target but NOT the public internet (the egress lock, FR-17 AC4 / NFR-03).
"""
