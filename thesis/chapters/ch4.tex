...
copied by hand into \LaTeX{} is a second description of the system, and the
second description is the one that silently goes stale.
...
that a later change had to keep satisfying. Table~\ref{tab:requirements}
...
FR-02 & Ingest structured reports (JSON) and manual entry & Implemented \\ \hline
...
FR-15 & Evaluation harness & Implemented \\ \hline
...
FR-19 & CVSS and MITRE ATT\&CK enrichment & Implemented \\ \hline
NFR-01 & Verdict reliability & Measured by the FR-15 harness \\ \hline
...
NFR-04 & Data protection & Author's direct responsibility \\ \hline
...
\begin{figure}[!htb]
\centering
\includegraphics[height=0.78\textheight]{figs/arch-containers.pdf}
...
\paragraph{One door to the database.} \texttt{db.py} is the only module that
talks to SQLite. Every other module receives an open session and works through
the mapped classes. The benefit is not purity but auditability: a question like
``what writes a verdict row?'' has a bounded answer.
...
one, because a budget measures effort and what matters is whether a conclusion
...
\paragraph{What it cost, and what was kept.} Replacing it deleted roughly a
thousand lines of working, tested code together with four requirements' worth of
implementation, and it was done in deliberate slices with the old path kept
...
decision to delete it was made after the second design was demonstrably better on
the same targets, and it was recorded --- with its rationale, its alternatives
and its consequences --- in the architecture decision record that supersedes the
four it replaces. That record, rather than the code that survived, is the most
accurate artefact of the design work this chapter describes.
