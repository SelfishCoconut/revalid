...
When the scope names a host that is not the lab, the session is confined instead by
a per-session \emph{egress gateway}: a helper container installs a packet-filter
allowlist for the scoped host's addresses and the sandbox runs inside that
gateway's network namespace, holding raw-socket access for its tools but not the
privilege to change the filter. Every tool --- not only HTTP clients --- can then
reach the scoped host and nothing else. Provisioning \emph{fails closed}: if the
gateway cannot be created the session is torn down rather than allowed to run with
open egress. The weaker property should be stated plainly --- in that second
configuration the boundary is a correctly configured allowlist rather than an
absent route, a promise again, if a small and auditable one held in a container the
agent cannot reach; and because it filters by address, it pins the scoped host's
addresses when the session starts and cannot follow a host that moves across a
rotating content-delivery network.
