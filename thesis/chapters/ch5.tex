...
gated run's 32.5. That row differs from the gated one in \emph{two} respects rather than
one --- the gate is gone \emph{and} the backend is a hosted \texttt{claude-sonnet-5}
rather than the local \texttt{qwen3.6:27b} --- so it compares two conditions rather than
isolating either.

What the comparison does say is the more useful finding. Two changes that should each have
cut the time sharply --- removing a per-command round-trip, and replacing a 27~B local
model with a hosted frontier one whose per-step latency is an order of magnitude lower ---
...
spending roughly twice as many commands on the finding it still could not confirm as the
gated run had, so its per-step advantage went into extra commands instead of into
finishing earlier. The accuracy difference points the same way: autonomy confirmed the
basket IDOR the tightly-scoped gated run had hedged, seven of nine becoming eight, which
on the evidence of Section~\ref{sec:eval-verdicts} is the hosted backend at work rather
than the absent gate.

The design conclusion follows directly. Nothing measured here shows the approval gate to
be the bottleneck, and the lever that visibly moves wall-clock is the reasoning backend,
so the place to spend for speed is a stronger model rather than a weaker safety contract.
And on the four findings the tool cannot yet decide it still defers to the human rather
than guessing, which is the same conservative contract the reliability result showed.
...
gate, so no claim about the gate's price in wall-clock is drawn from
it. \emph{Scale:} one report, twelve findings, one operator, which is
...
Three things follow from this study. \emph{The pipeline works
...
that the future work of Chapter~\ref{ch:conclusions} takes up. The summary is a tool
already trustworthy in the direction that matters and measurably improvable in the
direction that is not: the safety contract is a property of the design and held on
whichever backend was loaded, while the accuracy is a property of the model behind it
and rises with a stronger one.
