...
apparatus and a first result; a larger study across more reports, more targets and a
stronger model backend is set out as future work in
Section~\ref{subsec:future-eval}. What this chapter claims is feasibility and a
direction, not a final figure.
...
Appendix~\ref{ch:annex-install}: OWASP Juice Shop, pinned to version~17.1.1, a
deliberately vulnerable application. The input is a real penetration-test report ---
...
hedges, which each ran their full effort budget: the eight findings the system
...
and training terms apply --- a class of exposure that surveys of language-model privacy
treat as inherent to hosted inference \cite{das2024security}. Running both the
...
dependency, and with it the exposure to a single vendor's decisions, which the
literature on open versus closed foundation models frames as distributing rather than
concentrating control \cite{bommasani2021foundation, kapoor2024openfoundation}. The
