...
backend for both the gated runs and the verdict scoring was a locally hosted model
(Ollama, \texttt{qwen3.6:27b}), configured with a 16\,K-token context window, so the
accuracy figures below are a \emph{lower bound} on what the system achieves with a
stronger reasoning backend, a point the discussion returns to. One further condition, the
autonomous run of Section~\ref{sec:eval-time}, was made on a \emph{hosted} model
(\texttt{claude-sonnet-5}); because that changes the backend and the approval gate at the
same time, it is read with the care Section~\ref{sec:eval-time} sets out and not as a
...
\begin{table}[htbp]
\caption[Four retest conditions]{Four ways to retest: the author's lean manual pass;
the gated system on all twelve findings; the gated system on the nine non-browser
findings only (excluding the three --- stored and reflected scripting, and template
injection --- whose effect it cannot observe over HTTP); and those same nine run
autonomously with the approval gate removed. \textbf{The backends differ}: the two gated
rows ran on the local \texttt{qwen3.6:27b}, the autonomous row on a hosted
\texttt{claude-sonnet-5}, so rows~3 and~4 differ in \emph{two} respects and neither
isolates the cost of the gate. The manual figure is reported, not instrumented; the
machine figures are measured. The two nine-finding rows are \emph{separate runs}, not
...
Autonomous --- free-launch & \texttt{claude-sonnet-5} (hosted) & 9 & 29.7\,min & 8/9 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
gated run's 32.5. That is the row to read carefully, because it differs from the gated
run in \emph{two} respects rather than one: the gate is gone \emph{and} the backend is a
hosted \texttt{claude-sonnet-5} rather than the local \texttt{qwen3.6:27b}. It therefore
does not measure what the gate costs, and this chapter does not claim that it does.

What it does say is worth more than what it cannot. Two changes that should each have cut
the time sharply --- removing a per-command round-trip, and replacing a 27~B local model
with a hosted frontier one whose per-step latency is an order of magnitude lower ---
together bought about 9\%. That points away from both and at a third quantity: the number
of steps the agent chooses to take. Left unsupervised, the stronger model \emph{wandered},
spending twenty-two commands on the template-injection finding it still could not confirm
against the gated run's ten, so its per-step advantage went into extra commands instead of
into finishing earlier. The accuracy difference has to be read the same way. Autonomy
confirmed the basket IDOR the tightly-scoped gated run had hedged, seven of nine becoming
eight, but the backend changed at the same time, and the hosted model is the more likely
cause on the evidence of Section~\ref{sec:eval-verdicts}.

So the tempting conclusion --- that the human gate is nearly free, and the usual
speed-for-safety trade therefore inverted --- is \emph{not} established here, and it would
have been the most quotable result in the chapter. Isolating the gate needs the same
backend on both sides of the comparison, which this study does not have; it is recorded
among the threats to validity and is the cheapest of the follow-ups
Section~\ref{sec:eval-discussion} lists. What survives is a weaker claim that still bears
on the design: nothing measured here shows the gate to be the bottleneck, and the one
lever that visibly does move the time is the backend, so the place to spend for speed is a
stronger model rather than a weaker safety contract. And on the four findings the tool
cannot yet decide it still defers to the human rather than guessing, which is the same
conservative contract the reliability result showed.
...
these figures understate the system's ceiling. \emph{Confounded comparison:} the
autonomous condition of Table~\ref{tab:eval-conditions} changed the backend as well as the
gate, so this study cannot separate the cost of the approval gate from the speed of the
model, and no claim about the gate's price is made from it; repeating the free-launch run
on \texttt{qwen3.6:27b} would settle it in one afternoon. \emph{Scale:} one report, twelve findings, one operator, which is
...
that the future work of Chapter~\ref{ch:conclusions} takes up. A fourth thing this study
set out to say and \emph{cannot} belongs here too, since a result withheld is as
informative as one reported: \emph{what the approval gate costs is still unmeasured}. The
autonomous condition changed the backend along with the gate, so the two effects are not
separable, and repeating that run on \texttt{qwen3.6:27b} is the single cheapest
experiment this chapter leaves open. The honest summary is a
