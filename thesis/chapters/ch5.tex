...
Four things follow from this study. \emph{The pipeline works
...
that the future work of Chapter~\ref{ch:conclusions} takes up. And \emph{the gate is
close to free}: on the nine findings run both ways on the same backend, removing the
per-command approval saved under three minutes in thirty, because the cost of a retest
is the model's per-step latency rather than the human round-trip. The summary is a tool
already trustworthy in the direction that matters and measurably improvable in the
direction that is not: the safety contract is a property of the design, and holds on
whatever backend is loaded at no meaningful cost in time, while the accuracy is a
property of the model behind it and rises with a stronger one.
