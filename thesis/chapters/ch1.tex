...
closed and not merely hidden by an incidental change. This re-testing is performed
by hand, one finding at a time, on every remediation cycle, and it demands careful
judgement to avoid declaring a still-exploitable weakness ``fixed''. It is exactly
the kind of high-volume, judgement-laden, evidence-driven task at which a
well-supervised AI assistant could excel, yet it has been largely untouched by the
wave of automation described above. The manual nature of revalidation is examined in
detail in Section~\ref{subsec:revalidation}.
...
and the declaration of AI-tool use required by the regulation. The subsequent
chapters present the analysis and design of the system, the details of its
implementation,
the experimental evaluation of its verdict reliability against a real
penetration-test report, and, finally, the conclusions drawn from the work together
with the directions it opens for the future.
