...
Section~\ref{sec:eval-time} measures it: the gated system's machine time came out
comparable to a manual pass over the same findings, and the lever that visibly moves
wall-clock is the reasoning backend rather than the approval step. A fully unattended
mode exists as well and was measured; supervision is the default, not the only option.
