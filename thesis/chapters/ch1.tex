...
unautomated --- breach-and-attack-simulation and continuous-validation platforms
...
\item \bft{Develop an automated, human-supervised retesting system} that executes
  ...
  evidence-backed verdict --- \emph{still-open}, \emph{fixed} or
...
re-verifies the finding one gated command at a time --- the operator approves each
...
topology that enforces its safety constraints, its persistence model, the gated
agentic retest loop at its centre, and the earlier architecture that was built and
then deleted. Chapter~\ref{ch:conclusions}, finally, draws together what the work achieved and what
it taught, and sets out the directions it opens --- chief among them the quantitative
evaluation of the system's verdict reliability against a real penetration-test report
and of the effort it saves over manual retesting, which this work frames and equips
but leaves as its immediate next step. Finally, Appendix~\ref{ch:annex-install}
provides a step-by-step guide to installing and operating the system.
