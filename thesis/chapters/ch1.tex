...
Section~\ref{sec:design-retest} shows what the gate buys. The obvious objection is
that oversight must cost speed, which is exactly what the retested objective O4 was
able to check: Section~\ref{sec:eval-time} finds that removing the gate entirely
bought no meaningful time, because the bottleneck is the model's own per-step
latency. The autonomous mode still exists and was measured; it is the default that
changed, and it changed on evidence.
