...
heavily on AI was a deliberate choice, since it is what contemporary software practice
...
\subsection{Competencies covered}
\label{subsec:competencies-covered}

Stated against the competencies of the degree, this dissertation evidences the
following, each with the artefact that shows it rather than an assertion.

The two competencies the project proposal declared are IS5 and IS6, and they are the
ones this work is in the best position to claim.

\emph{IS5 --- identifying, assessing and managing the associated risks.} A tool that
executes attack commands against a live system is, itself, the risk it has to manage,
and the memoir treats that as its central engineering problem rather than as a
disclaimer. The risks were identified and each was answered by a structural control
whose limits are then stated: unauthorised reach, answered by containment through
network membership rather than by a check, with three limits of that boundary written
down (Section~\ref{subsec:stack}) instead of left implicit; a false clearance,
the failure mode that would make the tool dangerous at a compliance gate, answered
by a conservative verdict contract and measured for it (NFR-01,
Section~\ref{sec:eval-verdicts}); an unreviewed action, answered by the per-command
gate; exposure of the tool itself, answered by the loopback-only posture (NFR-03) and
by a four-layer scanning gate applied to its own code; and the risk that the model
misleads its operator, answered by an append-only trail from which every verdict is
re-derivable. The threats to validity of Section~\ref{sec:eval-threats} and the
declared limits of Section~\ref{subsec:future-observability} are the same discipline
applied to the evidence rather than to the artefact.

\emph{IS6 --- designing a solution that integrates ethical, social, legal and
economic considerations.} All four bear on this design and each changed it. The
\emph{ethical} commitment is the gate itself: the system may not touch a target
without a human decision, and may not declare a finding fixed on ambiguous evidence.
The \emph{legal} constraints shaped both the tool and the dissertation: the tool
because revalidation is an obligation imposed by standards and payment-industry rules
(Section~\ref{subsec:revalidation}) and because it must reach only an authorised
target; the dissertation because one of the evaluation reports carries no licence,
which is why it is credited and linked rather than redistributed. The \emph{social}
dimension is the privacy argument of Section~\ref{subsec:eval-local}: a
penetration-test report is among the most sensitive documents an organisation holds,
which is what makes a locally hosted model a first-class option rather than a
fallback. The \emph{economic} dimension is the same section's governance argument, that
a capability rented from one vendor is exposed to that vendor's decisions, together
and with the deliberate decision to keep the per-command gate as the default even
though Section~\ref{sec:eval-time} could not establish what it costs, on the reasoning
that an unmeasured overhead is the cheaper thing to carry than an unaudited command.

Turning to the competencies of the degree itself, the \emph{instrumental} ones
(INS01--INS05) are exercised across the whole
project: a requirements specification written before the code, with nineteen
functional and five non-functional requirements each carrying acceptance criteria
(Table~\ref{tab:requirements}); the analysis, design and modelling of the system in
Chapter~\ref{ch:design}, from the use-case and class models to the runtime topology;
its implementation in Python and TypeScript; and, in particular, the recording and
justification of every significant decision as one of forty-six architecture decision
records, which is where the reasoning behind the design, including the decisions
that were later reversed, is preserved rather than lost.

The \emph{systemic} competencies (SIS01--SIS10) are where the project's distinctive
work sits. Chapter~\ref{ch:design} designs and integrates a system of parts that were
not free to be arbitrary: a language-model layer that had to remain replaceable, a
persistence seam that had to survive a database change, an agent whose actions had to
be gated. The specialisation competency of the software-engineering track (CO19) is
evidenced by that same chapter together with the process of Chapter~\ref{ch:methodology}:
requirements, architecture, an enforced quality gate and a measured evaluation, applied
to a system built to a standard rather than to a demonstration. Security engineering in
particular is evidenced not by the topic of the work but by the containment argument of
Section~\ref{sec:design-runtime}, a control moved out of the program and into the
environment with its own limits stated, and by the four-layer scanning gate that
holds the tool to the standard it exists to check.

The \emph{personal and professional} competencies (PER03, PER04) are evidenced by the
work's supervision and its conduct: the project was carried out under two tutors, one
at the School and one at Airbus Helicopters, in an industrial setting, with the design
decisions defended to both; and the memoir is written in English, which is not the
author's first language, satisfying the second-language requirement (UCLM01) at the
same time.

The transversal UCLM competencies are covered as follows. Command of information and
communication technologies (UCLM02) is the artefact itself and the toolchain around it.
Correct written communication (UCLM03) is this memoir and the documentation the project
publishes. Ethical commitment and professional deontology (UCLM04) is the competency
this particular project could least afford to treat as a formality, and it is
evidenced twice: in the tool's design, which refuses to touch a target without a
per-command human decision and refuses to declare a finding fixed on ambiguous
evidence; and in the declaration of Section~\ref{sec:ai-declaration}, which states the
use of artificial intelligence in this work in full, including the sessions whose model
identity the audit trail does not record.
