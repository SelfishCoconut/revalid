...
hedged, no false clearance on any of them, and a machine time comparable to a manual pass.
The reasoning backend is what raises those figures, and a wider study across more reports
and targets is the direction they point. This closing chapter steps back from the artefact
...
with the deliberate decision to keep the per-command gate as the default --- a choice
Section~\ref{sec:eval-time} then vindicated by measuring its cost at under three minutes
in thirty, so that the control is carried for almost nothing.
