...
leave that network. The reference target, OWASP Juice Shop, is provisioned with:
...
Two consequences follow from that design and are worth stating before the first run.
Because the sandbox has no route off its own network, the target must be addressed
by its container name (\texttt{http://revalid-juice-shop:3000}) and never as
\texttt{localhost}, which inside the sandbox resolves to the sandbox itself; a retest
scoped to a \texttt{localhost} address --- which is what a report's stated endpoints
usually say --- will simply fail to connect. And because the sandbox cannot download
anything, every tool the agent may use has to be present in its image already, so
that image is built once, locally, before the first retest:
...
With the service and the lab running, a full revalidation is carried out from the
browser:
...
so the whole set can be executed anywhere with \texttt{make test-demos} --- which is
also what the continuous-integration gate runs on every change
(Section~\ref{sec:ci}).
