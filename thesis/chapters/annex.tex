...
\caption[The overview]{The overview (\texttt{/}): the determination ledger, the severity profile and the loaded reports.}
...
\caption[Manual report entry]{Manual report entry (\texttt{/new}), which bypasses the language model (FR-02, ADR-0020).}
\label{fig:ui-new}
\end{figure}
...
\caption[A report and its findings]{A report and its findings (\texttt{/reports/:id}).}
\label{fig:ui-report}
\end{figure}
...
\caption[The extract stage]{The extract stage (\texttt{/findings/:id/extract}), with the pipeline stepper above.}
...
\caption[The goal stage]{The goal stage (\texttt{/findings/:id/goal}): the editable retest goal and the target scope.}
\label{fig:ui-goal}
\end{figure}
...
returned, which is the evidence the verdict is pinned to, rather than the agent's
recollection of it. The two \texttt{operator\$} lines in that terminal are the operator
...
\caption[The approval gate]{The retest console suspended at the per-command gate (\texttt{/retest-sessions/:id}), FR-05 and FR-17.}
...
\caption[Verdict and captured evidence]{The console after the run concluded, showing the verdict, the adjudication controls and the evidence terminal.}
...
\caption[The verdict stage]{The verdict stage (\texttt{/findings/:id/verdict}), recording the determination and its rationale (FR-09).}
...
\caption[Choosing the backend]{The settings view (\texttt{/settings}), where the backend is chosen at runtime (FR-13).}
...
\caption[The reports assistant]{The reports assistant (\texttt{/chat}, FR-18).}
\label{fig:ui-chat}
\end{figure}
